...
namespace Task4 {
    void sendFromClient(SOCKET sock) {
        ...
        send(sock, (char*)arr, sizeof(arr), 0);
        std::cout << "Array sent to server: " << arrayToString(arr, 5) << std::endl;
    }

    void handleOnServer(SOCKET sock) {
        int arr[5];
        recv(sock, (char*)arr, sizeof(arr), 0);
...
namespace Task5 {
    ...
    void handleOnServer(SOCKET sock) {
        int clientNum;
        recv(sock, (char*)&clientNum, sizeof(clientNum), 0);
        int serverNum = 10;
...
namespace Task6 {
    ...
    void handleOnServer(SOCKET sock) {
        int num;
        recv(sock, (char*)&num, sizeof(num), 0);
...
void dispatchServerTask(int task, SOCKET sock) {
    switch (task) {
        ...
        default: std::cerr << "Invalid task number received." << std::endl; break;
    }
}
